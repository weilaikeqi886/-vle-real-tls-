...
get_latest_version() {
  # 直接写死版本号，跳过所有针对 GitHub API 的请求逻辑
  RELEASE_LATEST="v25.12.8"
  PRE_RELEASE_LATEST="v25.12.8"
  echo "info: Skiped API check, using version $RELEASE_LATEST"
  return 0
}
...
download_xray() {
  # 修改为你自己的 Release 附件直连地址
  local DOWNLOAD_LINK="https://github.com/weilaikeqi886/-vle-real-tls-/releases/download/v25.12.8/Xray-linux-${MACHINE}.zip"
  
  echo "Downloading Xray archive: $DOWNLOAD_LINK"
  if curl -f -L -o "$ZIP_FILE" "$DOWNLOAD_LINK"; then
    echo "ok."
  else
    echo 'error: Download failed from your repo!'
    return 1
  fi

  echo "Downloading verification file: ${DOWNLOAD_LINK}.dgst"
  if curl -f -L -o "${ZIP_FILE}.dgst" "${DOWNLOAD_LINK}.dgst"; then
    echo "ok."
  else
    echo 'error: Download dgst failed!'
    return 1
  fi
