...
clear
echo -e "${BLUE}==================================================${PLAIN}"
echo -e "${BLUE}       REALITY 1000台服务器批量交互部署初始化       ${PLAIN}"
echo -e "${BLUE}==================================================${PLAIN}"
...
# 3. 写入完整的 deploy.yml (按您的要求始终生成完整版)
cat << 'EOF' > deploy.yml
---
...
# 4. 交互式生成 hosts.ini
echo -e "${YELLOW}现在开始配置服务器信息。如果全部输入完成，请在 [IP地址] 处直接回车跳过。${PLAIN}"
...
COUNT=1
while true; do
    echo -e "\n${BLUE}--- 配置第 $COUNT 台服务器 ---${PLAIN}"
    
    read -p "请输入 IP 地址 (直接回车结束输入): " V_IP
    if [[ -z "$V_IP" ]]; then
        break
    fi

    read -p "请输入 SSH 端口 (默认 22): " V_PORT
    V_PORT=${V_PORT:-22}

    read -p "请输入 SSH 用户名 (默认 root): " V_USER
    V_USER=${V_USER:-root}

    read -p "请输入 SSH 密码: " V_PASS
    if [[ -z "$V_PASS" ]]; then
        echo -e "${RED}密码不能为空，请重新录入该台机器！${PLAIN}"
        continue
    fi
    ...
    echo -e "${GREEN}已添加: $V_IP:$V_PORT${PLAIN}"
    let COUNT++
done
...
# 5. 打印指引
echo -e "\n${GREEN}==================================================${PLAIN}"
echo -e "${GREEN}         ✅ 环境初始化与清单生成完成！${PLAIN}"
echo -e "${GREEN}==================================================${PLAIN}"
echo -e "${YELLOW}当前已录入 $((COUNT-1)) 台服务器。${PLAIN}"
echo -e "${BLUE}立即开始部署：${PLAIN}"
echo -e "cd ~/reality_batch && ansible-playbook -i hosts.ini deploy.yml -f 30"
